/*
 * referee_serial_port_protocol.c
 *
...
void makeCustomDataPacket(uint8_t packet[28]){
  ...
  unsigned char * bs1 = (unsigned char*)&CustomData.data1;
  unsigned char * bs2 = (unsigned char*)&CustomData.data2;
  unsigned char * bs3 = (unsigned char*)&CustomData.data3;
  ...
  Append_CRC16_Check_Sum(packet,28);
  //packet[OFFSET +19] = (uint8_t)(((0xFF00 & WholeCRC) >> 8) & 0xFF);
  //packet[OFFSET +20] = (uint8_t)((0x00FF & WholeCRC) & 0xFF);
...
void setFrameHeader(uint8_t header[7]){
  ...
  //CRCCheck
  header[0] = 0xA5; //sof
  header[2] = (uint8_t)(((0xFF00 & frameHeader.dataLength) >> 8) & 0xFF);
  header[1] = (uint8_t)((0x00FF & frameHeader.dataLength) & 0xFF);;
  header[3] = frameHeader.seq;//Seq
  header[4] = Get_CRC8_Check_Sum(&header[0], 5-1, 0xFF);//Append_CRC8_Check_Sum(header,5);
  //frameHeader.crc8Value = HeaderCRC;//CRC8Check
  header[6] = (uint8_t)(((0xFF00 & frameHeader.cmdId) >> 8) & 0xFF);
  header[5] = (uint8_t)((0x00FF & frameHeader.cmdId) & 0xFF);;
